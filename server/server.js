...
app.put('/users/:uid', (req, res) => {
  ...
  db.run("UPDATE users SET username = ?, email = ? ,password = ? WHERE uid = ? ", [username, email, password, uid], (err) => {
    if (err) {
      return res.status(500).json({ error: err.message });
    }
...
app.delete('/users/:uid', (req, res) => {
  const uid = req.params.uid;
  db.run("DELETE FROM users WHERE uid = ?", [uid], (err, result) => {
    if (err) {
      return res.status(500).json({ error: err.message });
    }
    else {
...
app.put('/tiles/stock/:tid', (req, res) => {
  const tid = req.params.tid;
...
app.post('/payments', (req, res) => {
  const createdOn = moment().format("Do MMMM YYYY");
  const { uid, oid, price, paymentType, address, phonenumber,stock,tid } = req.body;
  db.run(`
      INSERT INTO payment (uid, oid, price, paymentType, createdOn,address,phonenumber)
      VALUES (?, ?, ?, ?, ?,?,?)
    `, [uid, oid, price, paymentType, createdOn, address, phonenumber], function (err,response) {
    if (err) {
      return res.status(500).json({ error: err.message });
    }
    db.run(`UPDATE tiles SET stock = ? WHERE tid = ?`,[stock, tid], (err,result)=> {
      if (err) {
        return res.status(500).json({ error: err.message });
      }
      else{
...
app.get('/payments/:uid', (req, res) => {
  const uid = req.params.uid;
  db.all(`
    SELECT p.pid, p.uid, p.oid, p.price, p.paymentType, p.createdOn,
           o.tid, t.tname, t.image
    FROM payment p
    INNER JOIN orders o ON p.oid = o.oid
    INNER JOIN tiles t ON o.tid = t.tid
    WHERE o.uid = ?`,
    uid, (err, rows) => {
      if (err) {
        return res.status(500).json({ error: err.message });
...
app.get('/orders/tileInfo/:oid/', (req, res) => {
  ...
  db.get(`
      SELECT t.tname, t.type,t.price,t.image,t.stock
      FROM orders AS o
    ...
    res.json({ tname: row.tname, type: row.type, image: row.image, price: row.price, stock: row.stock });
  });
});
